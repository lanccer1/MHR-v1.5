...
class DomainFronter:
    ...
    def __init__(self, config: dict):
        ...
        # ==================== MULTI-GAS v1.5 (بهینه) ====================
        script_ids = config.get("script_ids") or config.get("script_id")
        if isinstance(script_ids, str):
            self._script_ids = [str(script_ids).strip()]
        elif isinstance(script_ids, list):
            self._script_ids = [str(sid).strip() for sid in script_ids if str(sid).strip()]
        else:
            self._script_ids = ["YOUR_APPS_SCRIPT_DEPLOYMENT_ID"]
        ...
        try:
            self._parallel_relay = max(1, min(int(config.get("parallel_relay", 1)), len(self._script_ids or [])))
        except:
            self._parallel_relay = 1

        if len(self._script_ids) > 1:
            log.info(f"Multi-GAS enabled → {len(self._script_ids)} Apps Script(s) | Parallel: {self._parallel_relay}")
        # ======================================================
    ...
    # ── Core relay with retry (Multi-GAS v1.5) ─────────────────────────────

    async def _relay_with_retry(self, payload: dict) -> bytes:
        """Relay with retry + smart blacklist on 502 / quota errors"""
        attempts = 2
        last_error = None

        for attempt in range(attempts):
            try:
                sid = self._script_id_for_key(payload.get("u"))
                ...
                if self._is_quota_or_server_error(result):
                    self._blacklist_sid(sid, "502 / Quota / Rate limit")
                    if attempt < attempts - 1:
                        log.info("Quota/502 detected → switching to next GAS")
                ...
                last_error = e
                sid = self._script_id_for_key(payload.get("u"))
                self._blacklist_sid(sid, type(e).__name__)
                if attempt < attempts - 1:
                    log.debug(f"Relay attempt {attempt+1} failed, retrying...")
        ...
        if last_error:
            raise last_error
        raise RuntimeError("All relay attempts failed")


    @staticmethod
    def _is_quota_or_server_error(raw_response: bytes) -> bool:
        """تشخیص ارورهای quota و 502 از پاسخ GAS"""
        if not raw_response:
            return True
        text = raw_response.decode(errors="replace").lower()
        indicators = [
            "502", "503", "504", "quota", "exceeded", "limit", "too many",
            "request limit", "سرویس در طول یک روز", "دفعات زیاد", "بیش از حد"
        ]
